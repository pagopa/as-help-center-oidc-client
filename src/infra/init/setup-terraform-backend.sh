...
S3_BUCKET_NAME="terraform-backend-${RANDOM_STRING}"
...
if [[ $(aws s3api list-buckets --region "${REGION}" --query "Buckets[].[Name]" --output text) = *"terraform-backend"* ]]; then
   S3_BUCKET_NAME=$(aws s3api list-buckets --query 'Buckets[?contains(Name, `terraform-backend`) == `true`].[Name][0][0]')
